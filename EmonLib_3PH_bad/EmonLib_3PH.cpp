...
//--------------------------------------------------------------------------------------
void EnergyMonitor::voltage(unsigned  int _inPinV, double _VCAL, double _PHASECAL, int _PHASE)
{
   inPinV = _inPinV;
   VCAL = _VCAL;
   PHASECAL = _PHASECAL;
...
void EnergyMonitor::calcVI(unsigned int crossings,unsigned int timeout)
{
  ...
  unsigned int numberOfPowerSamples = 0;                   //Needed because 1 cycle of voltages needs to be stored before use 
 ...
 //before calculation print
     //Serial.print("B");Serial.println(phaseShiftedV);
    //voltB=phaseShiftedV;
	//voltB=VoltB/numberOfPowerSamples;
    ...
    {                       // voltage samples to calculate instantaneous powers
      ...
      if( PHASE == 2 ) {
		...
		phaseShiftedV = storedV[(numberOfSamples-PHASE2-1)%PHASE3] 
					  ...
					  //Serial.print("            P2");Serial.println(phaseShiftedV);
      
	  //phase2P=phaseShiftedV;
	  }
      else if( PHASE == 3) {
        phaseShiftedV = storedV[(numberOfSamples+1)%PHASE3] 
                      + PHASECAL * (storedV[(numberOfSamples+2)%PHASE3] - storedV[(numberOfSamples+1)%PHASE3]);
					  //Serial.print("                           P3");Serial.println(phaseShiftedV);
					  //phase3P=phaseShiftedV;
      }
      else {  // PHASE==1
        phaseShiftedV = lastFilteredV 
					  ...
					  //Serial.print("                                          P1");Serial.println(phaseShiftedV);
      //phase1P=phaseShiftedV;
	  }
      //after callculation print
  ...
  //Serial.print("crossCount=");  Serial.println(crossCount);
    //Serial.print("Vbefore=");Serial.println(voltB);
	//Serial.print("P1=");Serial.println(phase1P);
    //Serial.print("P2=");Serial.println(phase2P);
    //Serial.print("P3=");Serial.println(phase3P);
  //-------------------------------------------------------------------------------------------------------------------------
